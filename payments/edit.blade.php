                ...
                <div class="card">
                    ...
                    <div class="card-footer">
                        ...
                        @if($payment->status == 'unpaid')
                            <button type="button" data-toggle="modal" data-target="#CompletePaymentModal"
                                    class="btn btn-icon icon-left btn-dark"><i
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-12 mb-3">
                                    <label
                                        for="transaction_id">{!! __('admin.transaction_id', ['default' => 'Transaction ID']) !!}</label>
                                    <input type="text" class="form-control" name="transaction_id"
                                           value="{{ $payment->transaction_id }}"
                                           placeholder="{!! __('admin.transaction_id', ['default' => 'Transaction ID']) !!}">
                                </div>
                            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="form-group col-md-12 col-12">
                            <label> {{ __('admin.data') }} - {{ $payment?->gateway['driver'] ?? __('admin.gateways') }}</label>
                            <textarea class="form-control" style="min-height: 200px;" disabled>{{ json_encode($payment->data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</textarea>
                        </div>
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('payments.complete', $payment->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>You can manually complete payments that are unpaid. On completion, the application will
                            attempt to mark the payment as complete and run the program the payment was created for.</p>
                            ...
                            <select class="form-control select2 select2-hidden-accessible" name="gateway" tabindex="-1"
                                ...
                                @foreach (App\Models\Gateways\Gateway::getActive('subscription') as $gateway)
                                    <option value="{{ $gateway->id }}"
                                            @if(isset($payment->gateway['id']) AND $payment->gateway['id'] == $gateway->id) selected @endif>{{ $gateway->name }}
                                        ({!! __('client.subscription') !!})
                                    </option>
                                @endforeach
                                @foreach (App\Models\Gateways\Gateway::getActive() as $gateway)
                                    <option value="{{ $gateway->id }}"
                                            @if(isset($payment->gateway['id']) AND $payment->gateway['id'] == $gateway->id) selected @endif>{{ $gateway->name }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"
                            id="refundModalLabel">{!! __('admin.refund_payment', ['default' => 'Refund Payment']) !!}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form action="{{ route('payments.refund', ['payment' => $payment->id]) }}" method="post">
                        ...
                        <div class="modal-footer">
                            ...
                            @if(isset($payment->gateway['refund_support']) AND $payment->gateway['refund_support'] AND $payment->status == 'paid')
                                <button type="submit"
                                        class="btn btn-primary">{!! __('admin.submit_refund', ['default' => 'Submit Refund']) !!}</button>
                        @endif
                    </form>
                </div>
            </div>
